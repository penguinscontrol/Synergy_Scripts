function [out, comp_c_sca, comp_t_del, save_new_err] = compute_synergies_init_guess( varargin )
...
%             eps{s}, out, coordinate, t_del(s,:));
        ftomin = @(coordinate) reonstruction_error_no_scale_gradient(...
            eps{s}, out, coordinate, t_del(s,:));
        coordinate = c_sca(s,:)';
...
%         eps, coordinate, c_sca, t_del, T);
    ftomin = @(coordinate) reconstruction_error_no_values_gradient(...
         eps, coordinate, c_sca, t_del, T);
    [new_out, new_err, exitflag, others] = minFunc(ftomin,coordinate,options);
    new_out(new_out < 0) = 0;
